  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (data.session?.user?.email) {
        ...
        // así que si el dueño quitaba el permiso 'habitue' a un user
        // logueado, el user seguía adentro hasta cerrar manualmente.
    ...
    <div className="min-h-screen bg-crema md:flex">
      ...
      <div className="flex-1 min-w-0 md:pl-60 flex flex-col min-h-screen">
        <header className="sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-ink/10 h-16 flex items-center gap-3 px-4 sm:px-6">
          <span className="md:hidden text-xl font-medium text-brand-600">habitué<span className="text-gold">.</span></span>
