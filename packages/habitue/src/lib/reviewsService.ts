// reviewsService — reseñas internas del local (las que dejan los clientes tras
// un pedido del marketplace/web o una visita). RPC anon fn_listar_reviews_publicas.
...
export async function listarReviews(localSlug: string | null): Promise<{ data: ReviewsResumen; error: string | null }> {
  ...
  return {
    data: {
      ...
      reviews: rows.map((r) => ({
        review_id: r.review_id, autor_nombre: r.autor_nombre, rating: r.rating,
