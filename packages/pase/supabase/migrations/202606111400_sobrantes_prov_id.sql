-- 202606111400_sobrantes_prov_id.sql
-- Fix: sobrantes output now includes prov_id and prov_nombre so the frontend
-- can match them to virtual bloques created by manual "Pertenece a proveedor"
-- assignments. Without this, payments for manually-assigned providers stayed
-- orphaned in "Sobran en PASE" instead of appearing under the provider block.
...
BEGIN
  ...
  -- ── Resultado ── CHANGED: sobrantes now include prov_id + prov_nombre
  SELECT jsonb_build_object(
    'extracto', COALESCE((
