-- Fix crear_factura_completa: el INSERT ... SELECT * forzaba factura_items.id=NULL
-- (override del default serial) → NOT NULL violation al guardar renglones.
-- Nunca se disparó porque Neko cargaba facturas sin items; el fix del Lector IA
-- (persistir p_items, 21-jul) lo expuso. Ahora se enumeran columnas sin id.

CREATE OR REPLACE FUNCTION public.crear_factura_completa(p_factura jsonb, p_items jsonb DEFAULT '[]'::jsonb, p_idempotency_key text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_items IS NOT NULL AND jsonb_typeof(p_items) = 'array' AND jsonb_array_length(p_items) > 0 THEN
    INSERT INTO factura_items (factura_id, producto, cantidad, unidad, precio_unitario, subtotal, tenant_id, materia_prima_id, descartado_conciliacion)
    SELECT factura_id, producto, cantidad, unidad, precio_unitario, subtotal, tenant_id, materia_prima_id, COALESCE(descartado_conciliacion, false) FROM jsonb_populate_recordset(
      NULL::factura_items,
      (
