-- 202606042100_pagar_sueldo_sin_cap_sobrepago.sql
-- ═══════════════════════════════════════════════════════════════════════════
-- pagar_sueldo: el sobrepago (redondeo para arriba) ahora queda registrado
-- en pagos_realizados con el MONTO REAL pagado (sin capear al total).
--
-- Lucas 04-jun: en Argentina ya casi no hay billetes chicos, así que al pagar
-- sueldos en efectivo se redondea para arriba. Decisión:
--   • En legajo / cajas / historial el pago debe figurar por el monto REAL
--     pagado ($10.000), no por el sueldo teórico ($9.900).
--   • El extra NO se contabiliza aparte: simplemente sale de la caja elegida
--     junto con el pago. Sin "saldo a favor" ni "deuda".
--   • El aguinaldo se sigue calculando sobre el sueldo real (total_a_pagar/12),
--     NO sobre lo pagado de más.
--
-- Hasta hoy la RPC capeaba pagos_realizados al total (v_nuevos_pagos := LEAST(
-- total, ...)) — herencia de la migración 202605201300, anterior al trigger
-- C4-F15. Pero el trigger _resync_liquidacion_pagos (202605222300) recalcula
-- pagos_realizados = SUM(-importe) de los movimientos SIN capear. O sea la RPC
-- y el trigger estaban en tensión: el movimiento sacaba $10.000 de la caja
-- pero la liquidación mostraba $9.900. Este cambio destapa el cap para que la
-- RPC quede ALINEADA con el trigger: pagos_realizados = monto real pagado.
--
-- ÚNICO cambio de lógica respecto de 202605292230:
--   v_nuevos_pagos := LEAST(v_total_a_pagar, v_ya_pagado + v_asignado_total)
--   →  v_nuevos_pagos := v_ya_pagado + v_asignado_total   (sin cap)
--
-- Todo lo demás (locks FOR UPDATE, pagado_at = p_fecha, multi-local, tenant
-- en idempotency, aguinaldo sobre total) se preserva igual.
-- ═══════════════════════════════════════════════════════════════════════════
