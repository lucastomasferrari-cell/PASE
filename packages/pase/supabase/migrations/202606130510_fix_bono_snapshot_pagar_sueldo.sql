-- 202606130510_fix_bono_snapshot_pagar_sueldo.sql
--
-- FIX #2 de la misma raíz que 130410: al portar pagar_sueldo desde 072100 (en vez
-- de la vigente 072300) se perdió la persistencia del snapshot del BONO en
-- rrhh_liquidaciones (columna `bono`, agregada en 072200). El bono SÍ entra en
-- total_a_pagar (el monto pagado es correcto), pero la columna `bono` quedaba en 0
-- → el recibo no mostraba la línea de bono. Fix: agregar `bono` al INSERT y al
-- UPDATE (revivir) usando el valor canónico (v_canon->>'bono'). Copia byte-exacta
-- de 130410 + esas dos líneas.
--
-- ─── (header original de 130410) ────────────────────────────────────────────
