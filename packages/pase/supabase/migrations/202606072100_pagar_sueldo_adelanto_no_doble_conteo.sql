-- 202606072100_pagar_sueldo_adelanto_no_doble_conteo.sql
--
-- BUG (caso Esteban Adrian, detectado 07-jun): el adelanto se contaba DOS VECES.
--
-- El frontend calcula `total_a_pagar` = bruto − adelantos (NETO; ver
-- lib/calculos/rrhh.ts: total_a_pagar = subtotal2 − adelantos − ...). Es decir,
-- el adelanto YA está restado del total que se guarda en la liquidación.
--
-- Pero pagar_sueldo, además, SUMABA el adelanto dentro de pagos_realizados
-- (v_asignado_total := v_asignado_cash + v_total_adelantos). Resultado: el
-- adelanto se restaba una vez (del total) y se sumaba otra (como pago) →
-- el sistema creía que se había pagado $X de más (= el monto del adelanto).
--   Caso Esteban: neto $1.134.167, transferencia real $227.091, adelanto
--   $150.000. El sistema decía pagos=$377.091 (227.091+150.000) → "falta
--   $757.076" cuando en realidad faltan $907.076 (= 1.134.167 − 227.091).
--
-- Además quedaba DESALINEADO con el trigger _resync_liquidacion_pagos (que
-- recalcula pagos_realizados = SUM de los movimientos de efectivo/transfer,
-- SIN el adelanto). O sea: la RPC y el trigger se contradecían, y cualquier
-- re-sync posterior pisaba el valor.
--
-- FIX: el adelanto NO se suma a pagos_realizados ni al chequeo de completitud.
-- Su único efecto es: (a) ya está restado de total_a_pagar (neto), y (b) se
-- marca descontado=true (sin cambios, más abajo). Queda alineado con el
-- trigger: pagos_realizados = solo efectivo/transferencia.
--
-- Cambios respecto de 202606071200 (resto idéntico):
--   * v_asignado_total := v_asignado_cash;   (antes: + v_total_adelantos)
--   * guard MONTO_INVALIDO: permite el caso "adelanto cubre todo" (cash 0 pero
--     hay adelanto que consumir / neto <= 0).
-- NO backfillea registros históricos (eso se hace aparte, caso por caso).
...
BEGIN
  ...
  -- Aguinaldo: total_a_pagar / 12. (NOTA: total_a_pagar es neto-de-adelanto;
  -- el aguinaldo idealmente iría sobre el bruto = subtotal2. Se deja como está
  -- en este fix para no mezclar cambios; se corrige en una migración aparte.)
  IF v_completa THEN
