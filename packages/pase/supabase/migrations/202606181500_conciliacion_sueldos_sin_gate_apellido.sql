-- 202606181500_conciliacion_sueldos_sin_gate_apellido.sql
-- Conciliación: los sueldos no cruzaban. El cruce exigía que el APELLIDO del
-- empleado apareciera en la descripción de la línea del extracto MP, pero MP
-- nunca trae el nombre (descripciones genéricas: "Transferencia enviada",
-- "Varios", "Egreso MP"). Resultado: ningún sueldo MP cruzaba jamás (quedaban
-- "falta" en el extracto + "sobran" en PASE).
-- Fix: en R1 (match por monto) la condición de candidato pasa de
--   (m.emp_apellido IS NULL OR descripcion ~* apellido)  →  (TRUE OR ...)
-- es decir, los sueldos matchean por monto+fecha como cualquier otro mov.
-- Si hay montos repetidos el cruce los deja en "amarillo" (el usuario elige),
-- igual que el resto. R2 (path por nombre) se deja igual (inofensivo).
-- Verificado contra prod (local 1, mayo): sueldos pasan de rojo_falta → verde.
...
DECLARE
  ...
  v_all_ext_idxs INT[];
  v_all_ext_count INT;
  v_all_ext_sum NUMERIC;
  v_all_pase_sum NUMERIC;
  v_all_pase_count INT;
BEGIN
  ...
  -- R0: match por REFERENCIA del extracto. Los movimientos creados desde la
  -- conciliación llevan la ref del extracto en el detalle ("· ref <ref>"). El
  -- match por ref es DEFINITIVO: desambigua montos repetidos y vincula el mov
  -- creado con su línea exacta (antes quedaban como "sobran"/"por elegir"). Lucas 17-jun.
  FOR v_fila IN SELECT * FROM _ce_ext WHERE estado = 'rojo_falta' AND referencia IS NOT NULL AND length(referencia) >= 6 ORDER BY idx LOOP
    SELECT COUNT(*), MIN(m.id) INTO v_cand_count, v_cand_id
    FROM _ce_mov m
    WHERE NOT m.usado
      AND m.fecha BETWEEN p_periodo_desde AND p_periodo_hasta
      AND ABS(m.importe - v_fila.monto) <= 1
      AND position(v_fila.referencia IN COALESCE(m.detalle, '')) > 0;
  ...
  -- R1: individual — PERÍODO ESTRICTO
  FOR v_iter IN 1..3 LOOP
    ...
    FOR v_fila IN SELECT * FROM _ce_ext WHERE estado = 'rojo_falta' ORDER BY idx LOOP
      ...
      WHERE NOT m.usado
        ...
        AND m.fecha BETWEEN p_periodo_desde AND p_periodo_hasta
        AND (
          TRUE /* apellido no exigido: MP no trae nombres (Lucas 18-jun) */
  ...
  -- R2: sueldos por nombre — PERÍODO ESTRICTO
  FOR v_fila IN SELECT * FROM _ce_ext WHERE estado = 'rojo_falta' ORDER BY idx LOOP
    SELECT COUNT(*), MIN(m.id),
           COALESCE(jsonb_agg(jsonb_build_object(
             'id', m.id, 'fecha', m.fecha, 'importe', m.importe,
  ...
  -- R3: combos (±4 días de la fila — sin cambio)
  ...
  -- R3.5: facturas/remitos pendientes (sin cambio)
  FOR v_fila IN SELECT * FROM _ce_ext WHERE estado = 'rojo_falta' ORDER BY idx LOOP
    IF v_fila.alias_tipo = 'gasto_directo' THEN CONTINUE; END IF;
  ...
  -- R4: bloques por proveedor — ahora con contexto completo
  IF p_match_agrupado THEN
    FOR v_prov IN
    ...
    LOOP
      ...
      -- Filas rojo_falta de este proveedor (las que no matchearon)
      ...
      -- Pagos no usados de este proveedor (los que no matchearon)
      ...
      -- CONTEXTO: transferencias de este proveedor que YA matchearon
      -- individualmente (verde/verde_agrupado). Informativo para que el
      -- usuario vea la foto completa del mes para este proveedor.
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'idx', e.idx, 'fecha', e.fecha, 'monto', e.monto,
        'descripcion', e.descripcion, 'estado', e.estado
      ) ORDER BY e.fecha), '[]'::jsonb)
      INTO v_ya_ext
      FROM _ce_ext e
      WHERE e.estado IN ('verde', 'verde_agrupado', 'verde_bloque', 'amarillo')
        AND (
          e.alias_prov = v_prov.prov_id
          OR (e.alias_prov IS NULL
              AND unaccent(UPPER(e.descripcion)) ~ v_regex)
        );

      -- CONTEXTO: pagos de este proveedor que YA se usaron en matches
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        'id', m.id, 'fecha', m.fecha, 'importe', m.importe, 'detalle', m.detalle
      ...
      INTO v_all_ext_count, v_all_ext_sum
      FROM _ce_ext e
      WHERE (
        e.idx = ANY(v_filas_bloque)
        OR (e.estado IN ('verde', 'verde_agrupado', 'verde_bloque', 'amarillo')
            AND (e.alias_prov = v_prov.prov_id
                 OR (e.alias_prov IS NULL AND unaccent(UPPER(e.descripcion)) ~ v_regex)))
      );

      SELECT COUNT(*), COALESCE(SUM(m.importe), 0)
      INTO v_all_pase_count, v_all_pase_sum
      FROM _ce_mov m
      ...
      ELSE
        ...
        UPDATE _ce_ext SET
          ...
          bloque = jsonb_build_object(
            ...
            'dif', v_dif,
            'movs', v_movs_bloque,
            'ya_matcheados_ext', v_ya_ext,
            'ya_matcheados_pase', v_ya_pase,
            'total_completo_ext', v_all_ext_sum,
            'total_completo_pase', v_all_pase_sum,
            'total_completo_dif', COALESCE(v_all_ext_sum, 0) - COALESCE(v_all_pase_sum, 0),
            'facturas_pendientes', (
  ...
  -- ── Alertas: movimientos FUERA del período pero dentro de ±10 días ──
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'mov_id', m.id, 'mov_fecha', m.fecha, 'mov_importe', m.importe,
    'mov_detalle', m.detalle, 'mov_prov', m.prov_nombre,
    'ext_idx', e.idx, 'ext_fecha', e.fecha, 'ext_monto', e.monto,
    'ext_descripcion', e.descripcion,
    'dias_fuera', CASE
      WHEN m.fecha < p_periodo_desde THEN (p_periodo_desde - m.fecha)
      ELSE (m.fecha - p_periodo_hasta)
    END
  ) ORDER BY e.idx), '[]'::jsonb)
  INTO v_alertas
  FROM _ce_ext e
  JOIN _ce_mov m ON NOT m.usado
    AND ABS(m.importe - e.monto) <= 1
    AND (m.fecha BETWEEN p_periodo_desde - 10 AND p_periodo_desde - 1
         OR m.fecha BETWEEN p_periodo_hasta + 1 AND p_periodo_hasta + 10)
  WHERE e.estado = 'rojo_falta';
