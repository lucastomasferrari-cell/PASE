-- 202606110800_fn_cruzar_r4_alias_sin_pagos.sql
-- Lucas 10-jun: 2 bugs en R4 del cruce:
--   (1) Solo iteraba proveedores que tienen pagos en _ce_mov. Aunque
--       hubiera alias de proveedor en filas del extracto, si no había
--       pagos en PASE, el bloque nunca se formaba → quedaban sueltos.
--   (2) Aunque la consulta encontraba la fila, si v_n=0 (sin pagos)
--       hacía CONTINUE → bloque no formado.
-- Fix combinado:
--   - El FOR v_prov ahora UNIONA proveedores con pagos + proveedores
--     referenciados por aliases en filas rojas.
--   - Cuando v_n=0 pero hay aliases apuntando al proveedor, se forma
--     un bloque con n_pagos=0 + dif=-suma_extracto (todo falta cargar).
-- Combinada con la mejora previa de fn_extraer_titular (sufijos SA/SRL/
-- etc. quitados), ahora proveedores como FALCON ESTEBAN CARLOS, BODEGAS
-- ESMERALDA, CONSULTORA UMAMI se agrupan directo, sin tener que asignar
-- manualmente.
...
BEGIN
  ...
  IF p_match_agrupado THEN
    FOR v_prov IN
      -- Proveedores con pagos en _ce_mov (caso histórico) UNION proveedores
      -- referenciados por aliases en filas rojas (caso "todavía no pagué a
      -- este proveedor en PASE pero la transferencia ya está en el extracto"
      -- — Lucas 10-jun: el matcher antes solo veía a los con pagos).
    ...
    LOOP
      ...
      -- Si el proveedor no tiene NINGÚN pago en PASE pero las filas del
      -- extracto SÍ apuntan a este proveedor por alias, igual formamos
      -- bloque (n_pagos=0, suma_pase=0). El bloque va a tener dif = -extracto
      -- y aparece como "Faltan cargar $X en PASE". Si tampoco hay alias
      -- (heurística por tokens fallida + 0 pagos), seguimos descartando.
      IF COALESCE(v_n, 0) = 0 THEN
        -- ¿Hay al menos una fila con alias_prov apuntando a este proveedor?
        IF NOT EXISTS (
