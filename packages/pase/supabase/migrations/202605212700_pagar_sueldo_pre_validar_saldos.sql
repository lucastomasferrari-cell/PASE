-- ═══════════════════════════════════════════════════════════════════════════
-- ALTO-9 Auditoría 2026-05-21: pagar_sueldo multi-local pre-validar saldos
--
-- Bug: el chequeo de saldo de cada cuenta/local se hacía dentro del loop de
-- inserts (línea por línea via _actualizar_saldo_caja). Si la línea 5 de 5
-- tiraba SALDO_INSUFICIENTE, plpgsql rollbackeaba todo (atomicidad correcta)
-- pero el dueño veía "saldo insuficiente" recién al final, después de haber
-- aprobado el pago completo.
--
-- Fix: Pass 1.5 nuevo entre Pass 1 (sumar montos) y Pass 2 (insertar movs).
-- Agrupa las líneas por (cuenta, local) en un jsonb, suma totales, y para
-- cada par verifica saldo_actual >= total_requerido. Si alguno está corto,
-- raise con detalle (qué cuenta + qué local).
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ★ Pass 1.5 (ALTO-9): pre-validar saldos por (cuenta, local).
  -- Agrupamos las líneas que pegan a la misma cuenta+local y validamos
  -- que el saldo actual cubra el total requerido. Si alguna no alcanza,
  -- abortamos ANTES de empezar a tocar nada → el dueño recibe error
  -- inmediato con qué cuenta/local está corto.
  FOR v_fp IN SELECT * FROM jsonb_array_elements(COALESCE(p_formas_pago, '[]'::jsonb)) LOOP
    v_monto := COALESCE((v_fp->>'monto')::numeric, 0);
    IF v_monto <= 0 THEN CONTINUE; END IF;
    ...
    -- Clave compuesta: "local_id|cuenta"
  ...
  -- Verificar saldo de cada (local, cuenta) acumulado.
