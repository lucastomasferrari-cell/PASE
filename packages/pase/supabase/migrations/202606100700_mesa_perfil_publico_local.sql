-- ════════════════════════════════════════════════════════════════════════
-- MESA · Sprint "de punta a punta" (09-jun) — perfil público del local.
--
-- 1) Campos de perfil en comanda_local_settings: descripción larga + fotos
--    (array de URLs; upload a Storage en v2).
-- 2) fn_get_perfil_publico_local(slug): UNA llamada anónima que devuelve todo
--    lo que la página pública necesita: info del local, horarios, "qué pedir"
--    (top items por ventas REALES de 30 días), resumen de reseñas, eventos
--    publicados, giftcards activas, locales hermanos del grupo, y el
--    "¿hay mesa ahora?" v1 (capacidad configurada).
-- ════════════════════════════════════════════════════════════════════════

ALTER TABLE comanda_local_settings
  ADD COLUMN IF NOT EXISTS mesa_descripcion TEXT,
  ADD COLUMN IF NOT EXISTS mesa_fotos JSONB NOT NULL DEFAULT '[]'::jsonb;
