-- 202606051200_pagar_sueldo_revive_liquidacion_anulada.sql
--
-- BUG (reportado 05-jun, caso DIAZ Josefina Maneki):
--   Anto anuló el pago de la Q1 de mayo para cambiar las formas de pago, lo
--   re-generó y al confirmar saltó "La liquidación está anulada"
--   (LIQUIDACION_ANULADA). La quincena quedaba TRABADA: imposible re-pagarla.
--
-- CAUSA:
--   pagar_sueldo, cuando no recibe p_liq_id, busca la liquidación de la novedad.
--   Si la encuentra, la usa; sólo inserta una nueva cuando NO existe ninguna.
--   Pero existe el constraint UNIQUE(novedad_id, cuota_num), así que no puede
--   haber dos liquidaciones para la misma quincena. Al anular, la liquidación
--   no se borra: queda anulado=true, estado='pendiente', pagos_realizados=0
--   (el _resync lo blanquea). Entonces pagar_sueldo la encuentra, NO inserta
--   otra (la unicidad lo impediría igual) y corta con LIQUIDACION_ANULADA.
--
-- FIX:
--   Si la liquidación hallada está anulada Y viene p_calc (re-generación del
--   pago), en vez de cortar se REUTILIZA la fila: se "revive" con los valores
--   nuevos y vuelve a estado 'pendiente' / pagos_realizados=0 / pagado_* NULL.
--   El movimiento anulado viejo queda como histórico. El aguinaldo ya se
--   revirtió al anular (anular_movimiento resta total/12), así que el nuevo
--   pago lo vuelve a sumar sin duplicar. Sin p_calc (no hay con qué recalcular)
--   se mantiene el corte LIQUIDACION_ANULADA como salvaguarda.
--
...
BEGIN
  ...
  -- FIX 05-jun: liquidación anulada -> REVIVIR en vez de cortar.
  -- El constraint UNIQUE(novedad_id, cuota_num) impide insertar otra para la
  -- misma quincena, así que reutilizamos la fila anulada reseteándola con los
  -- valores nuevos del pago corregido. Sin p_calc no hay con qué recalcular ->
  -- mantenemos el corte como salvaguarda. pagos_realizados ya está en 0 por el
  -- _resync del anular; el aguinaldo ya se revirtió en anular_movimiento.
