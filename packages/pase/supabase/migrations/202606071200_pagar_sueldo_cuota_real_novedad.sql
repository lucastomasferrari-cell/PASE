-- 202606071200_pagar_sueldo_cuota_real_novedad.sql
--
-- BUG (detectado 07-jun): pagar_sueldo es la UNICA funcion que inserta en
-- rrhh_liquidaciones, y su rama INSERT (cuando crea la liquidacion desde cero
-- con p_crear_liq=true) hardcodeaba cuota_num=1, cuotas_total=1 en vez de
-- heredar la cuota real de la novedad. Resultado: al pagar una Q2 que todavia
-- no tenia liquidacion, quedaba etiquetada "1/1" en vez de "2/2" -> recibos de
-- sueldo y detalle de movimientos con la quincena equivocada. En prod habia
-- 25+ liquidaciones con la cuota mal etiquetada vs su novedad.
--
-- FIX forward-only: la rama INSERT ahora lee cuota_num/cuotas_total de la
-- novedad (p_nov_id), con fallback a 1 si fueran NULL. NO toca plata. NO
-- backfillea las liquidaciones viejas mal etiquetadas (decision de Lucas: la
-- cuota historica es cosmetica y algunas novedades viejas tienen su propia
-- inconsistencia que requiere revision manual).
--
-- Conserva el fix 05-jun (revivir liquidacion anulada). Idempotente.
