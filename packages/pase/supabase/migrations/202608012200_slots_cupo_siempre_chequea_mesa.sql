-- Fix v2: turnos con cupo deben chequear mesas SIEMPRE, no solo por zona (01-ago)
-- ---------------------------------------------------------------------------
-- Bug (Camilo): el link público de Devoto mostraba el turno de las 20:00
-- DISPONIBLE aunque no hubiera ninguna mesa libre (todas ocupadas por las
-- reservas de las 20:30 que se solapan). El cupo tenía lugar (18 libres) pero
-- 0 mesas físicas. La versión anterior (202608011500) solo chequeaba mesas si
-- se elegía una zona puntual; para "cualquier lugar" miraba solo el cupo.
--
-- Fix: en el modo cupo, chequear la disponibilidad de mesas SIEMPRE (con o sin
-- zona). El cupo por turno es un tope ADICIONAL de pacing, pero siempre tiene
-- que existir una mesa/combinación libre para poder sentar. Cambio: la
-- condición `IF v_disp AND p_zona IS NOT NULL` pasa a `IF v_disp` en la rama
-- de cupo; el resto de la función queda idéntico.
  ...
  IF jsonb_typeof(v_turnos) = 'array' AND jsonb_array_length(v_turnos) > 0 THEN
    FOR v_turno_txt IN SELECT jsonb_array_elements_text(v_turnos) LOOP
      ...
      IF v_cupo IS NOT NULL THEN
        ...
        -- Fix 01-ago (v2): el cupo por turno es un tope ADICIONAL, no reemplaza
        -- las mesas físicas. SIEMPRE (elija zona o "cualquier lugar") tiene que
        -- haber una mesa/combinación libre — si no, aunque el cupo tenga lugar,
        -- no se puede sentar a nadie (bug Devoto 20:00: cupo con 18 libres pero
        -- 0 mesas disponibles → mostraba disponible en falso).
        IF v_disp THEN
