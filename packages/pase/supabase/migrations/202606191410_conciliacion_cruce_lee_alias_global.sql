-- Pieza 2a (continuacion): el cruce ahora LEE aliases globales (local_id NULL)
-- ademas de los del local, con prioridad al local-especifico. Necesario para
-- que los aliases tenant-wide (aprendizaje + fn_crear_alias_proveedor) se usen.
-- Verificado antes/despues en locales 1,2,3: totales identicos (no hay globales
-- todavia, los locales siguen matcheando igual).
...
BEGIN
  ...
  FROM (
    -- alias del local + alias globales (local_id NULL = todos los locales);
    -- el local-especifico tiene prioridad sobre el global (Pieza 2, 19-jun).
    SELECT DISTINCT ON (titular) titular, tipo, prov_id
    FROM conciliacion_alias
    WHERE tenant_id = v_tenant_id
      AND (local_id = p_local_id OR local_id IS NULL)
    ORDER BY titular, (local_id IS NULL)
  ) a
  ...
  -- R1-pre: match por PROVEEDOR (alias) + FECHA mas cercana. Para filas con
  ...
  -- pagos del mismo proveedor gana el de fecha mas cercana; empate -> amarillo.
  FOR v_fila IN
    SELECT * FROM _ce_ext
