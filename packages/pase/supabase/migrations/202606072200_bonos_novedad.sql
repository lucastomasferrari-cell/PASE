-- 202606072200_bonos_novedad.sql
--
-- Feature (pedido Lucas 07-jun): agregar "Bonos" como novedad del sueldo.
-- Es el espejo de "Otros desc. $": un monto manual que en vez de restar, SUMA
-- al total del sueldo de la quincena (premios, productividad, etc.).
--
-- - rrhh_novedades.bono (+ bono_motivo): lo que carga Anto en la grilla.
-- - rrhh_liquidaciones.bono: snapshot al confirmar/pagar (para el recibo).
-- - pagar_sueldo: guarda el bono en la liquidación (lee p_calc->>'bono').
--   El total ya viene calculado con el bono incluido desde el frontend
--   (lib/calculos/rrhh.ts: total_a_pagar = subtotal2 + bono − adelantos − ...).

ALTER TABLE rrhh_novedades
  ADD COLUMN IF NOT EXISTS bono NUMERIC NOT NULL DEFAULT 0 CHECK (bono >= 0),
  ADD COLUMN IF NOT EXISTS bono_motivo TEXT NULL;

COMMENT ON COLUMN rrhh_novedades.bono IS
  'Bono/premio manual que SUMA al total del sueldo de la quincena.';
COMMENT ON COLUMN rrhh_novedades.bono_motivo IS
  'Texto libre con la razón del bono (para auditoría / recibo).';

ALTER TABLE rrhh_liquidaciones
  ADD COLUMN IF NOT EXISTS bono NUMERIC NOT NULL DEFAULT 0;

COMMENT ON COLUMN rrhh_liquidaciones.bono IS
  'Snapshot del campo bono de la novedad al momento de confirmar/pagar.';

-- pagar_sueldo: idéntica a 202606072100 (adelanto sin doble conteo) + persiste
-- el bono en la liquidación (INSERT y rama "revivir anulada").
...
BEGIN
  ...
  -- Aguinaldo: total_a_pagar / 12. (total_a_pagar es neto; corrección a bruto
  -- queda flagueada para una migración aparte.)
  IF v_completa THEN
