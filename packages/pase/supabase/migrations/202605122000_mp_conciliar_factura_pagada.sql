-- ═══════════════════════════════════════════════════════════════════════════
-- fn_conciliar_mp_con_facturas: permitir conciliar contra factura ya pagada.
--
-- Bug reportado por Lucas 2026-05-12: la RPC rechazaba con FACTURA_YA_PAGADA
-- cuando se intentaba conciliar un egreso MP contra una factura cuyo estado
-- ya era 'pagada'. Caso operativo real: el egreso MP YA OCURRIÓ (la plata
-- salió de MP en producción) pero la factura ya había sido marcada como
-- pagada por otro medio antes. Resultado: la caja MP quedaba descuadrada
-- porque no había forma de imputar el egreso.
--
-- Fix: cuando la factura está pagada, permitir la conciliación pero:
--   - NO duplicar el pago en facturas.pagos[] (la factura ya tiene pagos
--     completos por su total — agregar más generaría sobrepago).
--   - NO mover el estado (ya estaba pagada).
--   - NO decrementar proveedores.saldo (ya se decrementó cuando se pagó
--     originalmente).
--   - SÍ registrar la fila en mp_movimiento_facturas (rastro auditable).
--   - SÍ generar el movimiento consolidado en caja MP (la plata salió real).
--
-- Esto significa: si concilás un MP $1000 contra una factura ya pagada,
-- la caja MP baja $1000 pero el saldo del proveedor no se mueve. La
-- coherencia contable queda a cargo del operador — si hubo doble pago,
-- tiene que generar manualmente un ingreso por la cuenta original.
-- Decisión consciente de Lucas.
-- ═══════════════════════════════════════════════════════════════════════════
