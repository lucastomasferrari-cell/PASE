-- ============================================================
-- Reservas: el cliente elige SECTOR (Barra/Salón/Terraza/Privado) y el motor
-- asigna una mesa DENTRO de ese sector. Zona opcional (NULL = cualquiera).
--   fn_zonas_reservables_publico → sectores disponibles para el cliente (anon)
--   fn_buscar_mesas_reserva      → + filtro por zona
--   fn_check_disponibilidad_reserva + fn_crear_reserva_publica → + p_zona
-- ============================================================

-- Sectores reservables de un local (para el selector del cliente).
CREATE OR REPLACE FUNCTION public.fn_zonas_reservables_publico(p_local_slug text)
RETURNS TABLE(zona text, capacidad_max integer)
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT m.zona, MAX(m.capacidad)::int
  FROM mesas m
  JOIN comanda_local_settings cls ON cls.local_id = m.local_id
  WHERE cls.slug = p_local_slug AND cls.tienda_activa = TRUE AND cls.deleted_at IS NULL
    AND m.deleted_at IS NULL AND m.reservable AND COALESCE(m.capacidad,0) > 0
    AND m.zona IS NOT NULL AND btrim(m.zona) <> ''
  GROUP BY m.zona
  ORDER BY m.zona;
$$;
GRANT EXECUTE ON FUNCTION public.fn_zonas_reservables_publico(text) TO anon, authenticated, service_role;

-- Buscar mesa(s) con filtro opcional por zona.
DROP FUNCTION IF EXISTS public.fn_buscar_mesas_reserva(integer, timestamptz, integer, integer, boolean);
...
BEGIN
  ...
  IF p_permite_combinar THEN
    -- Combo dentro de la MISMA zona (o cualquiera si no se pidió zona).
...
-- Disponibilidad con zona opcional.
...
-- Alta pública con zona opcional (auto-asigna mesa dentro del sector elegido).
DROP FUNCTION IF EXISTS public.fn_crear_reserva_publica(text, text, text, text, timestamptz, integer, text, text);
