-- ═══════════════════════════════════════════════════════════════════════════
-- Equipo: control de acceso por app (PASE/COMANDA/MESA/Habitué/Cocina)
-- 25-jun-2026
--
-- Lo gestiona el dueño desde Equipo. Cada app del ecosistema chequea esta
-- columna al login: si la app NO está en `apps_permitidas`, bloquea con
...
COMMENT ON COLUMN usuarios.apps_permitidas IS
  'Lista de apps del ecosistema a las que el usuario puede entrar. Valores: pase|comanda|mesa|habitue|equipo. Gestionado desde la app Equipo por el dueño.';

CREATE INDEX IF NOT EXISTS idx_usuarios_apps_permitidas
  ON usuarios USING GIN (apps_permitidas);

-- Auditoría de cambios de accesos (futuro: Equipo escribe acá cada vez que
-- el dueño cambia permisos/apps/locales de un usuario).
CREATE TABLE IF NOT EXISTS equipo_audit (
...
ALTER TABLE equipo_audit ENABLE ROW LEVEL SECURITY;

CREATE POLICY "equipo_audit_by_tenant" ON equipo_audit
  FOR ALL TO authenticated
  USING  (tenant_id = auth_tenant_id()::text)
  WITH CHECK (tenant_id = auth_tenant_id()::text);

CREATE INDEX IF NOT EXISTS equipo_audit_usuario_idx
  ON equipo_audit (tenant_id, usuario_id, created_at DESC);
...
BEGIN
  ...
  ASSERT (SELECT COUNT(*) FROM information_schema.tables
          WHERE table_name = 'equipo_audit') = 1,
         'equipo_audit no creada';
END;
$$;
