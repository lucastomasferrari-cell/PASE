-- ─────────────────────────────────────────────────────────────────────────
-- Fix eliminar_tenant_completo: desabilitar trigger append-only de
-- ventas_pos_overrides (descubierto 29-may corriendo CI repetidamente).
-- ─────────────────────────────────────────────────────────────────────────
--
-- Síntoma: el cleanup E2E del CI (globalSetup → cleanupE2ETenant) fallaba
-- con "Stale tables tras vuelta 3: {locales,canales,items,ventas_pos_items,
-- ventas_pos,rrhh_empleados}", impidiendo correr ningún test.
--
-- Diagnóstico (29-may sesión Lucas):
--  - El RPC ya desabilita 3 triggers append-only: auditoria_no_update/delete,
--    sync_saldos_caja, sync_pagos_rrhh, y los user triggers de recetas_versiones.
--  - PERO `ventas_pos_overrides` (historial inmutable de overrides POS — manager
--    autorizó comp/transferir mesa/etc) tiene su propio trigger
--    `trg_overrides_no_modify` con función `fn_overrides_immutable()` que
--    prohíbe DELETE. La RPC no lo desabilita → la tabla queda con filas zombie.
--
-- Cadena de bloqueo verificada:
--   ventas_pos_overrides ← imposible borrar (trigger)
--     bloquea ventas_pos_items (FK ventas_pos_overrides_venta_item_id_fkey)
--       bloquea ventas_pos (FK ventas_pos_items_venta_id_fkey)
--         bloquea canales + items (FK ventas_pos_canal_id_fkey + item_id_fkey)
--           bloquea locales + rrhh_empleados (cascada total)
--
-- Fix: agregar 1 DISABLE + 1 ENABLE de trg_overrides_no_modify en la RPC,
-- siguiendo el mismo patrón que los otros 3 triggers append-only.
-- ─────────────────────────────────────────────────────────────────────────
...
COMMENT ON FUNCTION public.eliminar_tenant_completo IS
  'Elimina tenant completo + todas las tablas con tenant_id. Fix 2026-05-29: '
  'agregado DISABLE/ENABLE de trg_overrides_no_modify (ventas_pos_overrides) — '
  'el 4to trigger append-only que faltaba desabilitar y bloqueaba el cleanup E2E.';
