-- Guard de sobrepago: bloquea pagar_sueldo cuando el total de pagos
-- superaría el total adeudado por más de un 2%. Tolerancia del 2% para
-- cubrir redondeos legítimos. Caso Castaño: doble pago Q1 ($440k x2).
--
-- Único cambio vs la vigente (202606130510): bloque SOBREPAGO_EXCESIVO
-- entre el cálculo de v_asignado_total y el loop de movimientos.
...
BEGIN
  ...
  ELSE
    ...
    IF v_liq IS NULL THEN
      ...
      INSERT INTO rrhh_liquidaciones (
        ...
        COALESCE((v_canon->>'bono')::numeric, 0),
  ...
  IF v_liq.anulado IS TRUE THEN
    ...
    UPDATE rrhh_liquidaciones SET
      ...
      bono = COALESCE((v_canon->>'bono')::numeric, 0),
  ...
  -- ★ GUARD SOBREPAGO: si el pago acumulado superaría el total adeudado
  -- por más de un 2%, bloquear. Tolerancia del 2% cubre redondeos legítimos
  -- pero impide el doble-pago (caso Castaño jun-2026).
  v_nuevos_pagos := v_ya_pagado + v_asignado_total;
  v_tolerancia := GREATEST(v_total_a_pagar * 0.02, 1);
  IF v_nuevos_pagos > v_total_a_pagar + v_tolerancia THEN
    RAISE EXCEPTION 'SOBREPAGO_EXCESIVO: pagos_acum=% total=% tolerancia=2%%',
      v_nuevos_pagos, v_total_a_pagar;
  END IF;
