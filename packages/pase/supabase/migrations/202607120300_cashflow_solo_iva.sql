-- 202607120300_cashflow_solo_iva.sql
-- Facturas solo_iva NO computan en el cashflow (espejo del EERR). Re-crea pyl+puente
-- excluyendo facturas con solo_iva=true de CMV / gastos / por-pagar. (Incluye el fix
-- de aguinaldo de 202607120200.)
-- pagar_aguinaldo/pagar_vacaciones dejan un movimiento cat='SUELDOS' con
-- pago_especial_id_ref (liquidacion_id NULL). Faltaban en:
--   - cashflow_pyl_mes DEVENGADO (réplica EERR) y PERCIBIDO (egresos por pago).
--   - cashflow_puente_mes DEVENGADO.
BEGIN;
...
BEGIN
  ...
  SELECT coalesce(SUM(total),0) INTO d_cmv FROM facturas
   WHERE tenant_id=v_tenant AND local_id=p_local_id AND fecha >= p_periodo_mes AND fecha < v_fin
     AND (bucket IS NULL OR bucket='cat_compra') AND (estado <> 'anulada' OR estado IS NULL) AND solo_iva = false;
  ...
  FROM facturas
  WHERE tenant_id=v_tenant AND local_id=p_local_id AND fecha >= p_periodo_mes AND fecha < v_fin
    AND (estado <> 'anulada' OR estado IS NULL) AND solo_iva = false;
  ...
  -- Aguinaldos/vacaciones (pago_especial_id_ref) — se cuentan en el mes de pago.
  d_sueldos := d_sueldos + (SELECT coalesce(SUM(-importe),0) FROM movimientos
      WHERE tenant_id=v_tenant AND local_id=p_local_id AND anulado=false AND cat='SUELDOS'
        AND pago_especial_id_ref IS NOT NULL AND fecha >= p_periodo_mes AND fecha < v_fin);
  ...
  FROM (
    SELECT abs(m.importe) AS a,
      CASE
        WHEN m.liquidacion_id IS NOT NULL OR m.pago_especial_id_ref IS NOT NULL THEN 'sueldos'
    ...
    WHERE m.tenant_id=v_tenant AND m.local_id=p_local_id AND m.anulado=false
      ...
      AND (m.fact_id IS NOT NULL OR m.gasto_id_ref IS NOT NULL OR m.liquidacion_id IS NOT NULL OR m.pago_especial_id_ref IS NOT NULL)
  ) x;
...
-- ── Puente: sumar aguinaldos al sueldo devengado ──────────────────────────
...
BEGIN
  ...
  v_sueldos := v_sueldos + (SELECT coalesce(SUM(monto),0) FROM gastos
      WHERE tenant_id=v_tenant AND local_id=p_local_id AND fecha >= p_periodo_mes AND fecha < v_fin
        AND tipo='empleado' AND (estado <> 'anulado' OR estado IS NULL));
  v_sueldos := v_sueldos + (SELECT coalesce(SUM(-importe),0) FROM movimientos
      WHERE tenant_id=v_tenant AND local_id=p_local_id AND anulado=false AND cat='SUELDOS'
        AND pago_especial_id_ref IS NOT NULL AND fecha >= p_periodo_mes AND fecha < v_fin);
  ...
  SELECT coalesce(SUM(total),0) INTO v_por_pagar FROM facturas
   WHERE tenant_id=v_tenant AND local_id=p_local_id AND fecha >= p_periodo_mes AND fecha < v_fin
     AND estado='pendiente' AND solo_iva = false;
