-- 202606222200_aguinaldo_preview_multilocal.sql
-- Agrega `multi_local` al preview de aguinaldos: true si el empleado está
-- asignado a más de un local (principal + cesiones). El masivo "Pagar
-- Aguinaldos" usa esto para NO pagarlos inline (no puede repartir entre
-- locales) y en su lugar mandar al legajo a pagarlos repartido (Lucas 22-jun).
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'multi_local',   (SELECT COUNT(*) FROM rrhh_empleado_locales rel
                       WHERE rel.empleado_id = e.id AND rel.deleted_at IS NULL) > 1,
