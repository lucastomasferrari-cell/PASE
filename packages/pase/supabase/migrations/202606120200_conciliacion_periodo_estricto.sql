-- 202606120100_conciliacion_periodo_estricto.sql
-- Cambio de ventana de matching: de ±15 días a período estricto.
-- Lucas: "si hay pagos del 06 capaz que hacen que no matchee lo pagado
-- en pase con mercadopago porque infla". Ahora:
--   - R1/R2: solo matchea movimientos dentro del período del extracto
--   - R3 combos: ±4 días de la fila (ya era así)
--   - R4 bloques: p_periodo_desde-4..p_periodo_hasta+4 (ya era así)
--   - Sobrantes: estricto al período
--   - NUEVO: campo "alertas" con movimientos fuera del período pero
--     dentro de ±10 días que coinciden por monto — informativo.
...
BEGIN
  ...
  -- Ventana agrupada sigue amplia para R3 combos y R4 bloques que
  -- necesitan movimientos de días cercanos al borde del período.
  ...
  -- R1: individual — PERÍODO ESTRICTO (antes ±15 días)
  ...
  -- R4: bloques por proveedor (sin cambio en ventana — ya usa p_periodo ±4)
  IF p_match_agrupado THEN
    FOR v_prov IN
  ...
  -- que coinciden por monto con alguna fila roja restante. Informativos,
  -- no se matchean automáticamente. ──
