-- 202606072400_bloquear_sobrepago_excesivo.sql
--
-- SEGURIDAD (Lucas 08-jun, caso Alexia): "pagás el mes donde estás parado, por
-- lo que ese mes debe — no puede ser de otra manera". El sobrepago de redondeo
-- (pagar un poco de más) sigue permitido, pero un sobrepago GRANDE casi siempre
-- es plata de OTRO MES cayendo en el mes equivocado (Alexia: $908k de junio
-- pagados en mayo) o un aumento no cargado. Se frena en el backend (defensa en
-- profundidad; el frontend ya avisa/bloquea).
--
-- Único cambio vs 202606072300 (aguinaldo sobre bruto): se agrega el guard
--   IF v_sobrepago > 5000 THEN RAISE EXCEPTION 'SOBREPAGO_EXCESIVO'.
-- 5000 es holgado para redondeo (los redondeos reales son < $1000).
...
BEGIN
  ...
  -- Freno de sobrepago grande: ver cabecera. Redondeo OK, plata de otro mes NO.
  IF v_sobrepago > 5000 THEN
    RAISE EXCEPTION 'SOBREPAGO_EXCESIVO';
  END IF;
