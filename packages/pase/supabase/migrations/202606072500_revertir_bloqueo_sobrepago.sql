-- 202606072500_revertir_bloqueo_sobrepago.sql
--
-- REVERT (Lucas 08-jun): el guard de la migración 072400 NO es lo que Lucas
-- quiere. Su modelo: cada mes es un bucket independiente. Si estás parado en un
-- mes y pagás, va a ese mes — la plata que salió es la que salió, sea de más o
-- de menos. NO se acumula ni se arrastra entre meses, y NO se bloquea por pagar
-- de más. Cualquier ajuste por un mes anterior va en las novedades, no es un
-- acumulado automático.
--
-- Esta restaura pagar_sueldo SIN el guard (= versión 072300: adelanto sin doble
-- conteo + aguinaldo sobre el bruto). El sobrepago vuelve a permitirse libre.
