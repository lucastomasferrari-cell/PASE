...
test.describe("Insumos", () => {
  ...
  test("lista con columnas esperadas", async ({ page }) => {
    ...
    expect(hText).toContain("UNIDAD");
  });

  test("buscador filtra", async ({ page }) => {
  ...
  test("botón Nuevo Insumo abre modal con opciones de unidad", async ({ page }) => {
    const btn = page.locator("button", { hasText: "Nuevo Insumo" });
    ...
    expect(modalText).toContain("PESO");
    expect(modalText).toContain("VOLUMEN");
    expect(modalText).toContain("UNIDAD");
  });
});
