...
export function MovimientoCajaDialog({ open, onOpenChange, tipo, onConfirmado }: Props) {
  ...
  const requiereOverride = tipo === 'ajuste' && Math.abs(monto) > UMBRAL_AJUSTE_OVERRIDE;
  ...
  async function ejecutar() {
    ...
    if (motivo.trim().length < 5) { toast.error('Motivo: mínimo 5 caracteres'); return; }
    setSaving(true);
    const { error } = await registrarMovimiento(localId, empleado.id, tipo, monto, metodo, motivo.trim());
    setSaving(false);
  ...
  return (
    <>
      <Dialog open={open && !showOverride} onOpenChange={onOpenChange}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={intentarConfirmar} disabled={saving || monto <= 0 || motivo.trim().length < 5}>
      ...
      <ManagerOverrideDialog
        ...
        descripcion={`Ajuste de ${formatARS(monto)} con motivo: "${motivo}".`}
        onAuthorized={async () => { await ejecutar(); }}
      />
    </>
  );
}
