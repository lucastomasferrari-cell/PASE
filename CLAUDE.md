...
- **Regla: actualizar memoria al cerrar features, sprints, o bugs significativos** (acordado 2026-05-21). Lucas no lleva registro mental de lo que se hizo y depende de que la memoria persistente esté al día. Si terminás algo concreto y tangible (sprint completo, feature nueva end-to-end, bug gordo resuelto, migration aplicada, etc.), **antes de cerrar la sesión** agregalo a un archivo en `C:\Users\lucas\.claude\projects\C--Users-lucas\memory\` — sea creando uno nuevo (`project_pase_features_YY_MM.md` o similar) o agregando una entrada al archivo correspondiente (típicamente `project_tareas_manuales_pendientes.md` si hay acciones pendientes, o `project_pase_features_NN_NN_mes.md` si es feature/sprint). Después actualizar el índice `MEMORY.md` si el archivo es nuevo. Sin esto, cuando Lucas pregunta "qué pendientes hay" termino mencionando cosas ya hechas semanas atrás. Heurística: si tuviste que escribir más de 3 commits para algo, vale la pena un párrafo de memoria.
- **Regla: cada feature nueva o modificación toca el test E2E full** (acordado 2026-05-22 noche). Existe la suite `packages/pase/tests/e2e-full/` que ejecuta ~70 operaciones encadenadas contra un tenant aislado "E2E Test Suite" + verifica invariantes SQL al final + corre en CI en cada push. **Cada vez que se agrega una feature nueva, se modifica una existente, o se descubre un bug**, la suite E2E full debe actualizarse en el mismo commit/PR. Patrón: si el cambio agrega un nuevo flow → agregar operación nueva al script del "mes operativo"; si cambia comportamiento de flow existente → ajustar asserts; si descubre un bug → agregar invariante SQL nuevo o operación que reproduzca el caso. Sin esto la suite deja de reflejar la realidad y el test mutante chico ya no alcanza para coverage end-to-end. **NO se hace merge a main si la suite E2E falla.**
- **Regla: registrar TODO pendiente manual de Lucas EN EL MISMO TURNO que se genera** (acordado 2026-05-21, Lucas: "muchas veces no lo hago y capaz queda perdido y nunca lo termino haciendo o dejamos cosas a medias"). Cada vez que en una sesión queda algo que Lucas tiene que hacer manualmente (configurar env var en Vercel, agregar secret en GitHub, aplicar SQL en Supabase, hacer smoke test, tomar decisión de producto, conseguir credenciales de un partner, hacer setup en panel externo de Meta/MP/etc, mergear datos duplicados con Anto, etc.), **agregarlo INMEDIATAMENTE al archivo `C:\Users\lucas\.claude\projects\C--Users-lucas\memory\project_tareas_manuales_pendientes.md`** — no esperar al final de la sesión, no asumir que Lucas se lo va a acordar, no postergar para "cuando consolide memoria". El archivo tiene un INDEX al principio con categorías (Urgente / Setup técnico / Decisiones de producto / Big-tickets postergados). Cuando Lucas reporta que completó algo, marcarlo ✅ y dejarlo histórico o moverlo a la sección "✅ Hechos". El archivo es PINNED en MEMORY.md → se lee cada vez que Lucas pregunta "qué tengo pendiente".
...
- **C3 — `applyLocalScope` obligatorio.** Toda query nueva sobre tabla con `local_id` usa `applyLocalScope(q, user, localActivo)` de `src/lib/auth.ts`. Bloqueante con ESLint rule `pase-local/require-apply-local-scope` (heurística file-level: el archivo debe importar `applyLocalScope` o usar un filtro manual `.eq("local_id", ...)`/`.in("local_id", ...)`). Tablas cubiertas: `ventas`, `gastos`, `facturas`, `factura_items`, `remitos`, `movimientos`, `saldos_caja`, `conceptos_caja`, `mp_movimientos`, `mp_credenciales`, `mp_justificaciones`, `conciliaciones_mp`, `rrhh_empleados`, `rrhh_liquidaciones`, `rrhh_adelantos`, `rrhh_pagos`, `caja_movimientos_categorias`. Excepciones: tests, scripts, lecturas cross-local intencionales (con `// eslint-disable-next-line pase-local/require-apply-local-scope -- motivo`).
...
- **C8 — Lazy imports en `App.tsx`.** Toda página nueva con `lazy(() => import(...))` + `<Suspense>`. Bloqueante con ESLint rule `pase-local/no-eager-page-import-app` (solo en `src/App.tsx`). Login queda eager por diseño (entry point sin sesión).
