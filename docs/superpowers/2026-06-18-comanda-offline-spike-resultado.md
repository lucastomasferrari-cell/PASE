...
3. **🎯 Impedancia RPC vs tabla — CONFIRMADO con datos (decisión central del rebuild):** un motor local-first sincroniza escribiendo **tablas** directo, pero **eso NO funciona acá**. Introspección (18-jun) de las columnas NOT NULL sin default: `ventas_pos` exige **`numero_local`, `modo`, `canal_id`** y `ventas_pos_pagos` exige **`idempotency_key`** — son campos que **solo la lógica de negocio sabe llenar** (ej. `numero_local = MAX(numero_local)+1` por local, que computan las RPCs `fn_*_comanda_offline`). Un upsert crudo del motor fallaría. **Conclusión firme: el push/upload del motor DEBE llamar a las RPCs `_offline` existentes** (que ya calculan esos campos + resuelven uuid→bigint), no escribir tablas. Vale igual para RxDB y PowerSync (patrón "backend connector" de PowerSync / push handler de RxDB que invoca RPCs). (items: ✓ no le falta ninguna columna obligatoria.)
