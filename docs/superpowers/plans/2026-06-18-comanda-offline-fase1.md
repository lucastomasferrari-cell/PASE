...
> - ✅ **T6b — bridge UI** (commit 7df5a4f): `tempId.ts` (uuidToTempId hash determinístico, no se guarda), `bridge.ts` (abrir/get/listItems/agregar/cobrar + mappers `VentaDoc→VentaPos`/`ItemDoc→VentaPosItem` + resolveUuid por escaneo), `repos.ts` separa agregarPago/marcarVentaCobrada, `OfflineProvider.watchReconcile` emite `comanda:reconcile-id`. `ventasService`/`pagosService` ramas offline (abrir, getVenta/listItems id<0, agregarItem, cobrar) → bridge. **VALIDADO EN BROWSER (preview, IndexedDB real):** abrir→2 ítems (total 19000 recalc)→cobrar→cobrada, tempIds negativos, sin errores de consola. Tests: `tempId.test.ts` (3) + `repos.test.ts` + mutante verdes. Scope = MOSTRADOR; mandarCurso/descuento/anular/mesa-ops siguen en sistema viejo → Fase 2.
> - ⏳ **T8 (RESTANTE) — solo Lucas:** prender flag (`localStorage comanda.ff.offline_first_ventas=1`) y hacer smoke logueado en mostrador (abrir→agregar→cobrar + ver que sincroniza). El engine + bridge ya están probados; falta el smoke end-to-end con sesión real (login + PIN). Después: Fase 2.
>
