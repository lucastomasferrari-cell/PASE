...
- **Lector de Facturas IA: arquitectura y modelo**. Pipeline: subir factura (JPG/PDF) → enviar **directo** al modelo `claude-opus-4-7` vía `/api/claude` (proxy transparente con `ANTHROPIC_API_KEY`) → parsear JSON → 3 capas de defensa post-IA (threshold de magnitud >\$10M ARS, coherencia items vs neto, coherencia desglose vs total) → preview con campos editables coloreados por confianza → human-in-the-loop confirma y guarda. **El pre-procesamiento de imagen browser-side (binarización Otsu / cam-scanner) probó empeorar la precisión** vs Opus 4.7 nativo en facturas argentinas con tinta gris claro y tipografía fina (smoke test bug #41: 92% confianza con original vs 55% con procesada). Por eso el pipeline NO procesa la imagen — la manda cruda al modelo. Si en el futuro aparecen facturas escaneadas con calidad muy baja donde Opus falle, considerar **Google Document AI** como fallback (endpoint serverless dedicado + service account GCP), no pre-procesamiento browser-side. Las defensas post-IA quedan como red de contención independiente del modelo.
